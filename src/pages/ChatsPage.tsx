    ...
    <div className="grid min-h-[calc(100dvh-9rem)] gap-4 md:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[360px_minmax(0,1fr)]">
      <CardBase className="min-w-0 p-3 md:h-[calc(100dvh-6rem)] md:overflow-y-auto">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black uppercase text-[var(--text-text)]">Mensagens</h1>
          </div>
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          {chatAtivo ? (
            <>
                ...
                {chatAtivo.mensagens.map((item) => {
                  ...
                  return (
                    <div key={item.id} className={`flex ${minhaMensagem ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[82%] rounded-2xl border px-4 py-3 ${minhaMensagem ? 'border-[var(--bg-primary)] bg-[var(--bg-primary)] text-[var(--text-primary)]' : 'border-[var(--border-border)] bg-[var(--bg-muted)] text-[var(--text-text)]'}`}>
              ...
              <form onSubmit={enviarMensagem} className="flex gap-2 border-t border-[var(--border-border)] p-3">
                <input value={mensagem} onChange={(evento) => setMensagem(evento.target.value)} className="input-form rounded-full px-4 py-3" placeholder="Digite sua mensagem..." />
        ...
        <CardBase className="min-w-0 p-4">
          ...
          <div className="mt-4 rounded-2xl border border-[var(--border-border)] bg-[var(--bg-muted)] p-3">
            ...
            <p className="mt-2 text-xs leading-5 text-[var(--text-muted)]">{selecionados.length ? `${selecionados.length} pessoa(s) escolhida(s).` : 'Escolha uma pessoa para chat direto ou várias para grupo.'}</p>
          </div>
        </CardBase>
      </div>
    </div>
