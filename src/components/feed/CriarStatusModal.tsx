...
export function CriarStatusModal({ aberto, onFechar }: CriarStatusModalProps) {
  ...
  return (
    <Modal aberto={aberto} titulo="Criar Status Orbitlink 24h" onFechar={onFechar} telaCheiaMobile>
      <div className="grid min-w-0 gap-4 lg:grid-cols-[1.15fr_.85fr] lg:gap-5">
        ...
        <div className="space-y-4 rounded-[1.5rem] border border-[var(--border-border)] bg-[var(--bg-muted)] p-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setLocalStatus('terra')} className={`rounded-2xl border px-2 py-3 text-[11px] font-black uppercase tracking-[0.04em] sm:px-3 sm:text-xs sm:tracking-[0.1em] ${localStatus === 'terra' ? 'border-[var(--bg-primary)] bg-[var(--bg-primary)] text-[var(--text-primary)]' : 'border-[var(--border-border)] text-[var(--text-muted)]'}`}>
                Terra
              </button>
              <button onClick={() => setLocalStatus('espaco')} className={`rounded-2xl border px-2 py-3 text-[11px] font-black uppercase tracking-[0.04em] sm:px-3 sm:text-xs sm:tracking-[0.1em] ${localStatus === 'espaco' ? 'border-[var(--bg-primary)] bg-[var(--bg-primary)] text-[var(--text-primary)]' : 'border-[var(--border-border)] text-[var(--text-muted)]'}`}>
                Céu
              </button>
            </div>
          </div>
